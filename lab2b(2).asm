...
# separated by a space.
		...
		syscall
		la	$a0,space		# load I/O register with value of space
		li	$v0,4			# system call code for print_int
		syscall
		lw	$a0,AnotherInt		# load I/O register with value of AnInt
		li	$v0,1			# system call code for print_int
...
task6:
		...
		add	$s3,$s3,$t0		# increment output point
		sub	$s2,$s2,1		# decrement index variable
		j	task6	
...
task7:
		...
		ble	$s0,$zero,done7	        # test if done
		lbu	$a0,($s2)
		li	$v0,1			# system call code for print_int
		syscall
		la	$a0,space		# load I/O register with value of space
		...
		sw	$t0,($s2)
		lbu	$a0,($s2)
		add	$t0,7		# increment input pointer
		sub	$s0,$s0,1		# decrement index variable
		j	task7
